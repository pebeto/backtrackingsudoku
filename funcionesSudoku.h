...
bool buscarMinMatriz(int i, int x, int y, int sudoku[9][9]){
    ...
    if(x%3==0 && y%3==1)                                    // La forma más eficiente de encontrar la posición
        for(int p=1, q; p<3; p++)                           // donde se encuentra un número en la celda de
            for(q=0; q<3; q=q+2)                            // una matriz, es calculando el módulo de su posición.
                if(sudoku[x+p][y-1+q]==i)                   //        _____________________________
                    return true;                            //       | i%3==0  | i%3==0  | i%3==0  |
    if(x%3==0 && y%3==2)                                    // i=0   | j%3==0  | j%3==1  | j%3==2  |
        for(int p=1, q; p<3; p++)                           //       |_________|_________|_________|
            for(q=0; q<2; q++)                              //       | i%3==1  | i%3==1  | i%3==1  |
                if(sudoku[x+p][y-2+q]==i)                   // i=1   | j%3==0  | j%3==1  | j%3==2  |
                    return true;                            //       |_________|_________|_________|
    if(x%3==1 && y%3==0)                                    //       | i%3==2  | i%3==2  | i%3==2  |
        for(int p=0, q; p<3; p=p+2)                         // i=2   | j%3==0  | j%3==1  | j%3==2  |
            for(q=1; q<3; q++)                              //       |_________|_________|_________|
                if(sudoku[x-1+p][y+q]==i)                   //
                    return true;                            //           j=0       j=1       j=2
    if(x%3==1 && y%3==1)                                    
        for(int p=0, q; p<3; p=p+2)                         // Tal como se aprecia en el gráfico mostrado, la
            for(q=0; q<3; q=q+2)                            // posicion de una celda puede ser definida por módulo.
    ...
    if(x%3==1 && y%3==2)                                    // su posición en el cuadro 3x3 local. Ejemplo:
        for(int p=0, q; p<3; p=p+2)                         // - Sabiendo que i y j son contadores que empiezan en 0:
            for(q=0; q<2; q++)                              //   Si sudoku[i][j], donde i= 6 y j= 7,
                if(sudoku[x-1+p][y-2+q]==i)                 //   i%3= 6%3= 0 y j%3= 7%3= 1, entonces su posición
                    return true;                            //   es (0,1) en su cuadro local.
    if(x%3==2 && y%3==0)                                    
        for(int p=0, q; p<2; p++)                           // Este método facilita el acceso rápido a las posiciones
            for(q=1; q<3; q++)                              // para no tener que almacenar una variable más, ocupando
    ...
    if(x%3==2 && y%3==1)                                    // El algoritmo que se ve al lado izquierdo lo denomino como
        for(int p=0, q; p<2; p++)                           // "método de sustracción obvia".
            for(q=0; q<3; q=q+2)                           
    ...
    if(x%3==2 && y%3==2)                                    // horizontales y verticales, puesto que ya han sido buscados.
        for(int p=0, q; p<2; p++)                           // Esto significa que en el peor de los casos, el algoritmo
            for(q=0; q<2; q++)                              // solo busca en 4 celdas, a diferencia del otro método que
